...
def helper_test_derivatives_translate_objects_random(testcase, objp0, objp1, objp2, smp0, smp1, smp2, n, objtol=1000):
    ...
    for _ in range(n):
        ...
        offsetdir1 = comm.bcast(offsetdir1, 0)
        ...
        offsetdir2 = comm.bcast(offsetdir2, 0)
...
def helper_test_derivatives_translate_object_fd(
    testcase, objp0, objp1, objp2, smp0, smp1, smp2, direction, value, test_first=True, objtol=1000
):
    result = g.compute(objp0, objp1, objp2, smp0, smp1, smp2, 0.001, 10, objtol, MPI.COMM_WORLD.py2f())
    perim = result[1]
...
def helper_test_derivatives_translate_object_given(
    testcase, objp0, objp1, objp2, smp0, smp1, smp2, direction, value, test_first=True, tol=1e-7, objtol=1000
):
    result = g.compute(objp0, objp1, objp2, smp0, smp1, smp2, 0.001, 10, objtol, MPI.COMM_WORLD.py2f())
    perim = result[1]
...
def helpter_test_derivs_cs(A1, B1, C1, A2, B2, C2, rho, testcase, indices_1=None, indices_2=None, method="cs"):
    maxdim = np.max(
        np.maximum(np.maximum(A2.max(axis=1), B2.max(axis=1)), C2.max(axis=1))
        - np.minimum(np.minimum(A2.min(axis=1), B2.min(axis=1)), C2.min(axis=1))
    )
    ...
    for i in range(p1_to_perturb.shape[1]):
        ...
        if method == "cs":
            for k in range(3):
                ...
                resultcs2 = gcs.compute(
                    A1_alt, B1_alt, C1_alt, A2, B2, C2, resultcs[2], rho, maxdim, MPI.COMM_WORLD.py2f()
                )
                ks_cs = resultcs2[0]
            ...
            for k in range(3):
                ...
                resultfd2 = g.compute(
                    A1_alt, B1_alt, C1_alt, A2, B2, C2, resultfd[2], rho, maxdim, MPI.COMM_WORLD.py2f()
                )
                ks_fd = resultfd2[0]
    ...
    testcase.assertTrue(
        tot_counter > 0,
        msg="If tot_counter remains zero there is some issue finding close points using the numpy slicing and searching",
    )
    ...
    for i in range(p2_to_perturb.shape[1]):
        ...
        if method == "cs":
            for k in range(3):
                ...
                resultcs2 = gcs.compute(
                    A1, B1, C1, A2_alt, B2_alt, C2_alt, resultcs[2], rho, maxdim, MPI.COMM_WORLD.py2f()
                )
                ks_cs = resultcs2[0]
                gradcs_ks = np.imag((ks_cs - ks_base)) / cseps
            ...
            for k in range(3):
                ...
                resultfd2 = g.compute(
                    A1, B1, C1, A2_alt, B2_alt, C2_alt, resultfd[2], rho, maxdim, MPI.COMM_WORLD.py2f()
                )
                ks_fd = resultfd2[0]
                gradfd_ks = (ks_fd - ks_base) / fdeps
...
class MinDistSTLTestCase1(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.maxdim = np.max(
            np.maximum(np.maximum(self.smp0.max(axis=1), self.smp1.max(axis=1)), self.smp2.max(axis=1))
            - np.minimum(np.minimum(self.smp0.min(axis=1), self.smp1.min(axis=1)), self.smp2.min(axis=1))
        )
    ...
    def test_derivs(self):
        ...
        helpter_test_derivs_cs(
            self.objp0, self.objp1, self.objp2, self.smp0, self.smp1, self.smp2, 300, self, [40, 10, 91], [47, 5, 90]
        )
...
class MinDistSTLTestCase2(unittest.TestCase):
    ...
    def test_derivs(self):
        helpter_test_derivs_cs(
            self.objp0, self.objp1, self.objp2, self.smp0, self.smp1, self.smp2, 300, self, [61, 10, 87], [51, 10, 12]
        )
...
class MinDistSTLTestCase3(unittest.TestCase):
    ...
    def test_derivs(self):
        helpter_test_derivs_cs(
            self.objp0, self.objp1, self.objp2, self.smp0, self.smp1, self.smp2, 300, self, [104, 87], [52, 2]
        )
...
class BisectSphereTestCase(unittest.TestCase):
    ...
    def test_two_cubes(self):
        ...
        objp0, objp1, objp2 = generate_plane(
            np.array([80.0, 0.0, 0.0]), np.array([0.0, 80.0, 0.0]), np.array([-40.0, -40.0, 0.00001])
        )
        result = g.compute(objp0, objp1, objp2, smp0, smp1, smp2, 0.001, 10, 100, MPI.COMM_WORLD.py2f())

        custom_assert(self, result[1], np.pi * 25.0, base_tol=5e-4)
        result2 = g.compute_derivs(objp0, objp1, objp2, smp0, smp1, smp2, 0.001, 10, 100, MPI.COMM_WORLD.py2f())
        custom_assert(self, result2[1], np.pi * 25.0, base_tol=5e-4)
        ...
        objp0, objp1, objp2 = generate_plane(
            np.array([80.0, 0.0, 0.0]), np.array([0.0, 80.0, 0.0]), np.array([-40.0, -40.0, 1e-4 + offset])
        )
        result = g.compute(objp0, objp1, objp2, smp0, smp1, smp2, 0.001, 10, 100, MPI.COMM_WORLD.py2f())
        ...
        helper_test_derivatives_translate_object_given(
            self, objp0, objp1, objp2, smp0, smp1, smp2, np.array([[0.0, 0.0, 1.0]]), deriv, test_first=True, tol=1e-2
        )
...
class BisectPlaneTestCase(unittest.TestCase):
    ...
    def test_bisect_planes(self):
        ...
        smp0, smp1, smp2 = generate_plane(
            np.array([0.0, 1.5, 0.0]), np.array([20.0, 0.0, 1.0]), np.array([-10.0, 0.0, 0.0])
        )
        result = g.compute(objp0, objp1, objp2, smp0, smp1, smp2, 0.001, 10, 100, MPI.COMM_WORLD.py2f())
        custom_assert(self, result[1], 1.5, base_tol=1e-3)

        smp0, smp1, smp2 = generate_plane(
            np.array([0.0, 100, 0.0]), np.array([20.0, 0.0, 1.0]), np.array([-10.0, -50.0, 0.0])
        )
        result = g.compute(objp0, objp1, objp2, smp0, smp1, smp2, 0.001, 10, 100, MPI.COMM_WORLD.py2f())
        custom_assert(self, result[1], 80.0, base_tol=1e-3)
...
class BisectCubeTestCase(unittest.TestCase):
    ...
    def test_plane_cube(self):
        ...
        helper_test_derivatives_translate_object_given(
            self, objp0, objp1, objp2, smp0, smp1, smp2, np.array([[1.0, 0.0, 0.0]]), 0.0, 100
        )
...
class OffsetSphereIntersectedTestCase(unittest.TestCase):
    ...
    def test_fixed_intersection(self):
        ...
        object_mesh = object_mesh_base.copy()
        offsetdir = np.array([0.0, 1.0, 0.0])
        sphere_rad = 25 / 2
        ...
        custom_assert(
            self,
            result[1],
            2 * np.pi * sphere_rad * np.sqrt(1 - (offsetmagnitude / 2 / sphere_rad) ** 2),
            base_tol=5e-3,
        )
    ...
    def test_randomly_generated_intersections(self):
        ...
        for _ in range(10):
            ...
            offsetvec = offsetmagnitude * offsetdir
            offsetvec = offsetvec.reshape((3, 1))
            offsetvec = comm.bcast(offsetvec, 0)
            offsetmagnitude = comm.bcast(offsetmagnitude, 0)
