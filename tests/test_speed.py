...
class BWBTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.objp2 = object_mesh[:, 2, :].transpose()
        self.maxdim = np.max(
            np.maximum(np.maximum(self.objp0.max(axis=1), self.objp1.max(axis=1)), self.objp2.max(axis=1))
            - np.minimum(np.minimum(self.objp0.min(axis=1), self.objp1.min(axis=1)), self.objp2.min(axis=1))
        )
