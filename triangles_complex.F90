module triangles
       use complexify 
    implicit none
    complex(kind=8) :: one = 1.0
    complex(kind=8) :: zero = 0.0
    contains

    subroutine dot_prod(d, v, w)
       use complexify 
        implicit none
        complex(kind=8), dimension(3), intent(in) :: v, w
    ...
    subroutine point_tri(a, b, c, p, dsquared)
       use complexify 
        implicit none
        complex(kind=8), dimension(3), intent(in) :: a, b, c, p
        complex(kind=8), intent(out) :: dsquared

        complex(kind=8), dimension(3) :: ab, ac, ap, bp, cp, closepoint, diff, dummydiff
        complex(kind=8) :: d1, d2, d3, d4, d5, d6
        complex(kind=8) :: v, vc, vb, va, w, denom
    ...
    subroutine clamp(n, min, max)
       use complexify 
        implicit none
        complex(kind=8), intent(in) :: min, max
        complex(kind=8), intent(inout) :: n
    ...
    subroutine line_line(p1, q1, p2, q2, dsquared)
       use complexify 
        implicit none
        complex(kind=8), dimension(3), intent(in) :: p1, q1, p2, q2
        complex(kind=8), intent(out) :: dsquared
        complex(kind=8), dimension(3) :: d1, d2, r, diff, c1, c2, dummy3
        complex(kind=8), parameter :: EPS = 1e-12
        complex(kind=8) :: a, b, c, e, f, s, t, denom
